        ...
        if "message" in data:
            ...
            if "text" in data["message"]:
                ...
                if action in ['adduser', 'au']:
                    ...
                    parser.add_argument('reason', type=str, default='無原因', nargs='?', help='原因')
                    parser.add_argument('-w', '--wiki', type=str, metavar='站點', help='參見 https://quarry.wmflabs.org/query/278 ，預設：zhwiki')
                    parser.add_argument('-r', '--reason', type=str, metavar='原因', default='無原因', help='預設：%(default)s')
                    parser.add_argument('-p', '--point', type=int, metavar='點數', default=10, help='預設：%(default)s')
                    ...
                    M.adduser_score(M.user_type(user), args.point, 'handler/cmd/adduser')
                    return 'OK'
                ...
                if action in ['setwiki']:
                    ...
                    parser = argparse.ArgumentParser(prog='/{0}'.format(action))
                    parser.add_argument('wiki', type=str, default='zhwiki', metavar='站點', help='參見 https://quarry.wmflabs.org/query/278 ，預設：zhwiki')
                    parser.add_argument('-u', '--user', dest='username', type=str, metavar='用戶名')
                ...
                if action in ['addpage', 'ap']:
                    ...
                    parser.add_argument('-w', '--wiki', type=str, metavar='站點', help='參見 https://quarry.wmflabs.org/query/278 ，預設：zhwiki')
                    parser.add_argument('-r', '--reason', type=str, metavar='原因', default='無原因', help='預設：%(default)s')
                    parser.add_argument('-p', '--point', type=int, metavar='點數', default=30, help='預設：%(default)s')
                    ...
                    M.addblack_page(page, m_date, reason, wiki=wiki, point=args.point)
                    return "OK"
                ...
                if action in ['massaddpage']:
                    ...
                    parser = argparse.ArgumentParser(prog='/{0}'.format(action))
                    parser.add_argument('pagetitle', type=str, nargs='+', help='頁面名')
                    parser.add_argument('-w', '--wiki', type=str, metavar='站點', help='參見 https://quarry.wmflabs.org/query/278 ，預設：zhwiki')
                    parser.add_argument('-r', '--reason', type=str, metavar='原因', default='無原因', help='預設：%(default)s')
                    parser.add_argument('-p', '--point', type=int, metavar='點數', default=30, help='預設：%(default)s')
